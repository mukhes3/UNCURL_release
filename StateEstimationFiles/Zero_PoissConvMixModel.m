function [M,Xconv,CostPerIter]   = Zero_PoissConvMixModel(Dat,MeansInit,k,eps,iterMax)
...
for i = 1:iterMax 
    ...
    [Xconv,~] = pConMix_runZero(Dat,M,x_init,Z);
    display(strcat('M step no. ',int2str(i)));     
    [temp, Cost] = pConMean_runZero(full(Dat'),full(Xconv'),full(M'),Z'); 
